...
tableChisq <- function(capt, stat, df, p) {  
  ...
  chi_param <- c(df)
  chi_p <- c(p)
  chi_df <- data.frame(chi_stat, chi_param, paste(chi_p,"*"))
  ...
  kable(chi_df,
        ...
        col.names = c("X-squared",
                      "df",
                      "p-value")) %>%
    footnote(symbol = "The Chi-squared statistic is significant at the 0.05 level")
}
```
